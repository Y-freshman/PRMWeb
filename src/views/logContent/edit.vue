  ...
  <div class="page-content">
    <el-card class="box-card">
      ...
      <el-form ref="list"  :model="list" label-width="100px">
        <el-form-item label="备注" prop="logsContent">
          <el-input   type="textarea"
                      :rows="3"
                      maxlength="150"
                      show-word-limit
                      placeholder="请输入备注内容" v-model="list.logsContent"/>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="box-card">
...
<script>
  import { selectlogsDetail,savelogsUpdate } from '../../api/logContent'
  export default {
    name: 'Edit',
    data () {
      return {
    ...
    methods: {
      ...
      updateComplete(){
        ...
        let t = 0;
        for (let i = 0; i < this.list.logsDetails.length; i++) {
            if(this.list.logsDetails[i].dContent=='' || this.list.logsDetails[i].dContent==null){
              this.$message.error({
                title: '失败',
                message: '任务不能为空'
              })
