    ...
    <el-card class="box-card">
      <el-form ref="list" :inline="true"  :model="list" label-width="100px" >
        ...
        <el-form-item label="填写时间" prop="logsDate">
          <el-date-picker v-model="list.logsDate"
                          type="date"
                          placeholder="选择日期"
...
<script>
  import { savelogsInsert,getTasksList,getUserListByExample,getplanValue,getUserList,getlogTask } from '../../api/logManagement'
  ...
  export default {
    ...
    methods: {
      getlogsdetailTask (){
        ...
        let yy = '';
        t = JSON.stringify(t);
        getlogTask(t).then(response => {
          this.userTask = response.data
          for (let i = 0; i < this.userTask.length; i++) {
            this.handleSpecificationShow()
            this.logsDetails.dPlanPrecent = this.userTask[i].planPercent;
            this.logsDetails.taskname = this.userTask[i].userTaskName;
            this.logsDetails.dActPrecent = this.userTask[i].userTaskActPercent;
            this.handleSpecificationAdd()
          }
          this.automatic()
        }).catch(() => {
          this.$message.error({
      ...
      getlogs (){
        this.list.logsDate = new Date();
        this.list.durTime = "8"
        this.list.username = getUser().name
      },
      getTasks(){
      ...
      handleSpecificationShow() {
        this.value1 = ''
        this.logsDetails = {  id: '', projectTaskId: '', logsId: '', dContent: '', dPlanPrecent: '', dActPrecent: '', dTaskType: '', memo: '',taskname:'' }
      ...
      handleUpdate(row){
        ...
        console.log(row)
        this.logsDetails.taskname = row.taskname
