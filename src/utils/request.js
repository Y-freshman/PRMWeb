...
const service = axios.create({
  ...
  // baseURL: 'http://192.168.91.216:8800', // api 的 base_url
...
service.interceptors.request.use(
  config => {
    ...
    let serviceName;
    if(config.hasOwnProperty('serverName')){
    ...
    if(process.env.hasOwnProperty(serviceName+'_API')){
      // config.baseURL = process.env.get_value(serviceName+'_API');
      config.baseURL = process.env[serviceName+'_API']+'/'+serviceName;
    }
    // if(process.env.hasOwnProperty(serviceName+'_GATEWAY')){
